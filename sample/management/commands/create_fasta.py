...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        sql = """SELECT sample_id, name
            ...
            'WHERE user_id=2' if opts['ena'] else ''
        )

        if os.path.exists(opts['output']):
            raise CommandError(
                f'{opts["output"]} exists already, exiting'
            )
        else:
        ...
        for sample in query_database(sql):
            ...
            with open(genes, 'w') as gene_out, open(proteins, 'w') as prot_out:
                for gene in get_genes_by_sample([sample['sample_id']], 2):
                    if gene['aa']:
            ...
            with open(contigs, 'w') as contig_out:
                for contig in get_assembly_contigs([sample['sample_id']], 2):
